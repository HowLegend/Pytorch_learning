''' 使用 gpu 训练的方法1:
首先在代码开头写：
device = torch.device("xxx")
其中“xxx”可以是:
cpu
cuda
cuda:0   /   cuda:1   /...     若有多个gpu，可以选择用哪一个
...
添加代码：
实例.to(device)          ---网络模型/损失函数
实例 = 实例.to(device)    ---数据（输入、标注）     网络模型/损失函数 也可以
'''
...
# 创建网络模型
tudui = Tudui()
tudui = tudui.to(device)  # 1. 网络模型：将网络模型放到cuda上，使用gpu训练

# 损失函数
loss_fn = nn.CrossEntropyLoss()  # CrossEntropyLoss: 交叉熵 损失
loss_fn = loss_fn.to(device)  # 2. 损失函数：将损失函数放到cuda上，使用gpu训练
...
for i in range(epoch):
    ...
    for data in train_dataloader:
        imgs, targets = data
        imgs = imgs.to(device)  # 3. 数据（输入、标注）：将数据（输入、标注）放到cuda上，使用gpu训练
        targets = targets.to(device)  # 3. 数据（输入、标注）：将数据（输入、标注）放到cuda上，使用gpu训练
